...
const aderenzaApprova = { verifica: async () => ({ aderente: true, motivi: [] }) };
const revisoreApprova = { revisiona: async () => ({ approvato: true, motivi: [] }) };
...
test("run: successo al primo tentativo, risultatiRicerca arriva al validatore come elenco di URL", async () => {
    ...
    const orchestrator = createNoteOrchestrator({
        generator, validator, revisore: revisoreApprova, aderenza: aderenzaApprova, writer,
...
test("run: nessuna fonte ufficiale -> si ferma subito senza ritentare", async () => {
    ...
    const revisore = { revisiona: async () => { throw new Error("non deve essere chiamato"); } };
    const aderenza = { verifica: async () => { throw new Error("non deve essere chiamato"); } };
    const writer = { write: async () => { throw new Error("non deve essere chiamato"); } };

    const orchestrator = createNoteOrchestrator({ generator, validator, revisore, aderenza, writer, logger: loggerSilenzioso, maxAttempts: 3 });
...
test("run: revisione mai approvata -> esaurisce i tentativi con motivo 'validation'", async () => {
    const generator = { generate: async () => ({ draft: { titolo: "x" }, risultatiRicerca: [] }) };
    const validator = { validate: (draft) => ({ success: true, data: draft }) };
    const revisore = { revisiona: async () => ({ approvato: false, motivi: ["fuori tema"] }) };
    const aderenza = { verifica: async () => { throw new Error("non deve essere chiamato"); } };
    const writer = { write: async () => { throw new Error("non deve essere chiamato"); } };

    const orchestrator = createNoteOrchestrator({ generator, validator, revisore, aderenza, writer, logger: loggerSilenzioso, maxAttempts: 2 });
    const risultato = await orchestrator.run("argomento");
...
test("run: bozza non aderente alle fonti -> ripete con feedback e poi ha successo", async () => {
    ...
    const validator = { validate: (draft) => ({ success: true, data: draft }) };
    const aderenza = {
        verifica: async (argomento, data) => {
            chiamateAderenza.push(data.titolo);
            return data.titolo === "bozza-1"
                ? { aderente: false, motivi: ["descrive un'API non presente negli estratti"] }
                : { aderente: true, motivi: [] };
        },
    };
    const writer = { write: async (data) => data };

    const orchestrator = createNoteOrchestrator({
        generator, validator, revisore: revisoreApprova, aderenza, writer,
        logger: loggerSilenzioso, maxAttempts: 3,
    });
    const risultato = await orchestrator.run("argomento");
    ...
    assert.equal(risultato.status, "success");
    assert.equal(risultato.attempts, 2);
    assert.deepEqual(chiamateAderenza, ["bozza-1", "bozza-2"]);
});

test("run: bozza mai aderente alle fonti -> esaurisce i tentativi con motivo 'validation'", async () => {
    const generator = { generate: async () => ({ draft: { titolo: "x" }, risultatiRicerca: [] }) };
    const validator = { validate: (draft) => ({ success: true, data: draft }) };
    const aderenza = { verifica: async () => ({ aderente: false, motivi: ["contenuto non supportato dalle fonti"] }) };
    const writer = { write: async () => { throw new Error("non deve essere chiamato"); } };
