...
function formatSourcesForReview(searchResults) {
    return searchResults
        .map((r) => {
            if (!r.content) return `- ${r.url}: nessun estratto disponibile (contenuto non recuperato)`;
            return `- ${r.url}:\n  """\n  ${r.content}\n  """`;
        })
        .join("\n\n");
}
...
// giudicare.
function buildReviewPrompt(topic, draft, searchResults) {
    const sectionsText = draft.sections
        .map((s) => `### ${s.title}\n${s.content}`)
        .join("\n\n");

    return `Di seguito trovi la bozza di un appunto sull'argomento "${topic}" e gli estratti delle fonti ufficiali usate per scriverla. Il tuo compito di valutazione — cosa giudicare e come — è spiegato per intero dopo questi due blocchi.

Titolo generato: "${draft.title}"
Modulo: "${draft.module}"
...
${formatSourcesForReview(searchResults)}
...
=== ASPETTO 1: PERIMETRO E LIVELLO (campo "scope") ===
1. PERIMETRO: il contenuto resta pertinente all'argomento richiesto "${topic}", senza divagazioni fuori tema?
...
Imposta "scope.approved" a true SOLO se non ci sono problemi di perimetro né sezioni che violano il criterio di livello sopra. Se approved è false, "scope.reasons" deve elencare in modo specifico e azionabile cosa correggere, un motivo per riga, massimo una frase ciascuno (es. "la sezione 'X' parla di Y, che non è pertinente a ${topic}", oppure "la sezione 'X' spiega a fondo Y, che rientra tra i punti sempre esclusi da un'introduzione: riducilo a un accenno o rimuovilo").

=== ASPETTO 2: ADERENZA ALLE FONTI (campo "adherence") ===
Controlla che i fatti, la sintassi e gli esempi scritti nella bozza siano supportati dagli estratti delle fonti sopra, NON dalla conoscenza pregressa di chi ha scritto la bozza sull'argomento "${topic}". Per le fonti senza estratto disponibile, la bozza deve restare generica sui loro dettagli specifici: va bene citarle come link, non va bene descriverne il contenuto come se l'estratto fosse stato letto.
...
Imposta "bestPractice.upToDate" a true SOLO se non usa sintassi o pattern che le fonti segnalano come superati. Se upToDate è false, "bestPractice.reasons" deve indicare in modo specifico e azionabile cosa sostituire e con cosa, un motivo per riga, massimo una frase ciascuno (es. "la sezione 'X' mostra Y, che le fonti segnalano come deprecato in favore di Z: usa Z").`;
}
...
function createReviewerAgent({ model, logger }) {
    async function review(topic, draft, searchResults) {
        try {
            ...
            const structuredModel = model.withStructuredOutput(ReviewSchema, { strict: true });
            const verdict = await structuredModel.invoke(
                buildReviewPrompt(topic, draft, searchResults)
            );

            if (!verdict.scope.approved) {
                logger.warn("reviewerAgent", "Bozza fuori perimetro o livello non adeguato", {
                    topic,
                    reasons: verdict.scope.reasons,
                });
            }
            if (!verdict.adherence.adherent) {
                logger.warn("reviewerAgent", "Bozza non aderente alle fonti", {
                    topic,
                    reasons: verdict.adherence.reasons,
                });
            }
            if (!verdict.bestPractice.upToDate) {
                logger.warn("reviewerAgent", "Bozza con pattern non aggiornati alle best practice", {
                    topic,
                    reasons: verdict.bestPractice.reasons,
                });
            }
            ...
            const agentError = new AgentError(
                `Revisione fallita per l'argomento "${topic}"`,
